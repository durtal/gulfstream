...
ggplot(racetypes, aes(x = race_type, y = n)) +
    ...
    labs(x = "Race Type", y = "Count", title = "Counts per Race Type (8.5f dirt)") +
    theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1, size = rel(1)))

ggplot(fieldsize, aes(x = field, y = count)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "No. of Runners", y = "Count", title = "Field Size (8.5f dirt)")
...
ggplot(data = tmp, aes(x = gate, y = pct_btn)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "Gate", y = "Pct Beaten", title = "Percentage of Rivals Beaten (8.5f dirt)")
...
tmp <- dirt %>%
    group_by(gate) %>%
    summarise(n = n(),
              mean_dist = mean(d8_5f, na.rm = TRUE))
...
ggplot(tmp, aes(x = factor(gate), y = d8_5f)) +
    geom_boxplot(fill = fill) +
    RcappeR::theme_rcapper() +
    labs(x = "Gate", y = "Distance (ft)", title = "Distance Raced from each Starting Gate (8.5f dirt)")
...
sspds <- select(wnrs, pos, ss2f:ss8_5f)
...
y$variable <- factor(x = y$variable, levels = c("2f", "4f", "6f", "7f", "7.5f", "8f", "8.5f"))

ggplot(subset(y, pos ==1), aes(x = variable, y = value)) + 
    geom_boxplot() + 
    geom_jitter(position = position_jitter(width = .4), color=fill, alpha=.1) + 
    RcappeR::theme_rcapper() +
    labs(x = "Sectional", y = "Speed (ft per sec)", title="Sectional Speeds of Winners (8.5f dirt)")

wnrs_ind <- which.max(wnrs$ss4f)
all_ind <- which.max(dirt$ss4f)
```

The fastest opening half mile by a horse who went on to win was by **`r toupper(wnrs$horse[wnrs_ind])`** on `r wnrs$date[wnrs_ind]` in race `r wnrs$race[wnrs_ind]`, recording a speed of **`r wnrs$ss4f[wnrs_ind]`** feet per second.  `r if(wnrs$horse[wnrs_ind] != dirt$horse[all_ind]) { paste("There were horses who ran faster, ", toupper(dirt$horse[all_ind]), " ran the opening 4f in a speed of ", dirt$ss4f[all_ind], " ft per second.  ", if(wnrs$date_race[wnrs_ind] == dirt$date_race[all_ind]) { paste("Both horses were competing in the same ", toupper(dirt$race_type[all_ind]), " race", if(dirt$runup[all_ind] >= 50) { paste(", potentially benefitting from a runup of ", dirt$runup[all_ind], " feet.", sep = "") } else { paste0(".") }, sep = "") }, sep = "") } else if(dirt$date_race[all_ind] != wnrs$date_race[wnrs_ind]) { paste("The same horse, ", toupper(wnrs$horse[wnrs_ind]), ", recorded the fastest opening 4f in victory and in defeat, running at", dirt$ss4f[all_ind], "ft per sec when finishing ", dirt$pos[all_ind], " in race ", dirt$race[all_ind], " on ", dirt$date[all_ind], ".", sep = "") } else { paste("This opening quarter was the fastest speed by any horse, in victory or defeat.") }`
...
names(tmp) = c("6.5f FinSpd", "4.5f FinSpd", "2.5f FinSpd", "1.5f FinSpd", "1f FinSpd", "0.5f FinSpd")
tmp
```
...
tmp2 <- tmp %>%
    ...
    summarise(c4f = round(mean(c4f, na.rm = TRUE),2),
              fs4.5f = round(mean(fs4_5f, na.rm = TRUE), 4))
...
The plots below show the distribution of 4.5f Finishing Speeds for winners compared to "The Rest", and the opening 4f sectionals for winners compared to "The Rest".  The distributions are similar, especially the opening 4f where winners don't really differentiate themselves to "The Rest" as we saw in dirt sprints.  Winners, on average, run the opening 4f in **`r win2f`**, compared to "The Rest", who run the opening 4f in **`r rest2f`**, a difference of **`r win_rest_diff`** seconds.  For reference, the horses leading their race after 4f run the opening 4f in an average time of **`r ldr2f`**.  It is the ability to carry that early speed to the finish that sees winners separate themselves from "The Rest".

```{r fig.show='hold', echo=FALSE, warning=FALSE, message=FALSE, out.height="300px", out.width="450px", fig.align='default'}
ggplot(tmp, aes(x = fs4_5f, y = ..density.. / 100, fill = Winner)) +
    geom_density(alpha = .5) +
    scale_fill_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) +
    RcappeR::theme_rcapper() +
    labs(x = "Finishing Speed", y = "", title = "4.5f Finishing Speed density plot (8.5f dirt)")

ggplot(tmp, aes(x = c4f, y = ..density.. / 100, fill = Winner)) +
    geom_density(alpha = .5) +
    scale_fill_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) +
    RcappeR::theme_rcapper() +
    labs(x = "Sectional (sec)", y = "", title = "Opening 4f Sectional density plot (8.5f dirt)")
```

Plotting the opening 4f sectional against the closing 4f Finishing Speeds the difference becomes a little more clear.  Unlike dirt sprints where Winners were seen to be running faster early, over `r dist` furlong races, horses over `r dist` furlongs have time to make up ground on tiring rivals more easily that they may have lost early .

```{r echo=FALSE, warning=FALSE, message=FALSE, out.height="400px", out.width="550px", fig.align='center'}
ggplot(tmp, aes(x = c4f, y = fs4_5f, col = Winner)) + 
    geom_point(alpha = .5) + 
    scale_color_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) + 
    RcappeR::theme_rcapper() +
    labs(x = "Sectional (sec)", y = "Finishing Speed", title = "Opening 4f vs 4.5f Finishing Speed (8.5f dirt)")

fastestfs <- which.max(wnrs$fs4_5f)
fastestop <- which.min(wnrs$c4f)

```

The winner with the highest 4.5f Finishing Speed was **`r toupper(wnrs$horse[fastestfs])`** when winning a **`r wnrs$race_type[fastestfs]`** (race `r wnrs$race[fastestfs]` on `r wnrs$date[fastestfs]`), after an opening 4f of **`r wnrs$c4f[fastestfs]`** `r toupper(wnrs$horse[fastestfs])` recorded a Finishing Speed of **`r wnrs$fs4_5f[fastestfs]`**.  The winner with the fastest opening 4f was **`r toupper(wnrs$horse[fastestop])`** when winning a **`r wnrs$race_type[fastestop]`** (race `r wnrs$race[fastestop]` on `r wnrs$date[fastestop]`), recording a time of **`r wnrs$c4f[fastestop]`** over the opening half mile and a 4.5f Finishing Speed of **`r wnrs$fs4_5f[fastestop]`**.
...
ggplot(racetypes, aes(x = race_type, y = n)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "Race Type", y = "Count", title = "Counts per Race Type (8.5f turf)") +
    theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1, size = rel(1)))

ggplot(fieldsize, aes(x = field, y = count)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "No. of Runners", y = "Count", title = "Field Size (8.5f turf)")
...
ggplot(data = tmp, aes(x = gate, y = pct_btn)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "Gate", y = "Pct Beaten", title = "Percentage of Rivals Beaten (8.5f turf)")
...
The draw will likely have an impact on the distance horses travel, especially as the first turn is such a short distance from the start.  Horses drawn wide (high) will likely be forced wide.  `r if(tmp$gate[which.min(tmp$mean_dist)] == 1) { paste("Unsurprisingly, horses") } else { paste("Horses") }` drawn in gate **`r lowgate`** travel the least distance, with an average trip of **`r lowdist`** feet, while horses draw in gate **`r highgate`** travel an average distance of **`r highdist`**, a difference of **`r highdist - lowdist`** feet (gates must have a minimum of 5 runs used).  The plot below shows boxplots of distance raced by horses in each starting gate (the large-ish number of outliers for each gate are likely due to rail adjustments).
...
ggplot(tmp, aes(x = factor(gate), y = d8_5f)) +
    geom_boxplot(fill = fill) +
    RcappeR::theme_rcapper() +
    labs(x = "Gate", y = "Distance (ft)", title = "Distance Raced from each Starting Gate (8.5f turf)")
...
Race shape on turf courses isn't quite as predictable as on dirt courses, as the surface allows horses who started slowly to finish faster.  The plot below shows the sectional speeds of winners, to be clear these aren't cumulative speeds, but the speed from one sectional to the next (taking into account the distance travelled - Trakus data - for each horse), so from the start to 2f, from 2f to 3f, etc.  Winners are still typically running slower over the final 0.5f but the fall off isn't as sharp as seen on dirt, and the middle part of the race from 2f to ~7.5f sees runners showing consistent speed.
...
ggplot(subset(y, pos ==1), aes(x = variable, y = value)) + 
    ...
    geom_jitter(position = position_jitter(width = .4), color=fill, alpha=.1) + 
    RcappeR::theme_rcapper() +
    labs(x = "Sectional", y = "Speed (ft per sec)", title="Sectional Speeds of Winners (8.5f turf)")
...
Finishing Speeds also reflect the consistent speed throughout the race, par Finishing Speeds for winners are:
...
The plots below show the distribution of 4.5f Finishing Speeds for winners compared to "The Rest", and the opening 4f sectionals for winners compared to "The Rest".  The distributions for the opening 4f are very similar, Winners, on average, run the opening 4f in **`r win2f`**, compared to "The Rest", who run the opening 4f in **`r rest2f`**, a difference of **`r win_rest_diff`** seconds.  For reference, the horses leading their race after 4f run the opening 4f in an average time of **`r ldr2f`**.  The 4f Finishing Speeds are also very similar.
...
ggplot(tmp, aes(x = fs4_5f, y = ..density.. / 100, fill = Winner)) +
    ...
    scale_fill_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) +
    RcappeR::theme_rcapper() +
    labs(x = "Finishing Speed", y = "", title = "4.5f Finishing Speed density plot (8.5f turf)")

ggplot(tmp, aes(x = c4f, y = ..density.. / 100, fill = Winner)) +
    geom_density(alpha = .5) +
    scale_fill_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) +
    RcappeR::theme_rcapper() +
    labs(x = "Sectional (sec)", y = "", title = "Opening 4f Sectional density plot (8.5f turf)")
```

Plotting the opening 4f sectional against the closing 4.5f Finishing Speeds the difference becomes a little more clear.  Runners aren't necessarily punished if they start slowly as in sprint, or dirt, races, the kinder surface means horses are capable of finishing much faster and making up ground on horses who started faster.
...
ggplot(tmp, aes(x = c4f, y = fs4_5f, col = Winner)) + 
    ...
    scale_color_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) + 
    RcappeR::theme_rcapper() +
    labs(x = "Sectional (sec)", y = "Finishing Speed", title = "Opening 4f vs 4.5f Finishing Speed (8.5f turf)")
...
The winner with the highest 4.5f Finishing Speed was **`r toupper(wnrs$horse[fastestfs])`** when winning a **`r wnrs$race_type[fastestfs]`** (race `r wnrs$race[fastestfs]` on `r wnrs$date[fastestfs]`), after an opening 4f of **`r wnrs$c4f[fastestfs]`** `r toupper(wnrs$horse[fastestfs])` recorded a Finishing Speed of **`r wnrs$fs4_5f[fastestfs]`**.  The winner with the fastest opening half mile was **`r toupper(wnrs$horse[fastestop])`** when winning a **`r wnrs$race_type[fastestop]`** (race `r wnrs$race[fastestop]` on `r wnrs$date[fastestop]`), recording a time of **`r wnrs$c4f[fastestop]`** over the opening half mile and a 4.5f Finishing Speed of **`r wnrs$fs4_5f[fastestop]`**.
