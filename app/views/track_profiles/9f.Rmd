...
ggplot(racetypes, aes(x = race_type, y = n)) +
    ...
    labs(x = "Race Type", y = "Count", title = "Counts per Race Type (9f dirt)") +
    theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1, size = rel(1)))

ggplot(fieldsize, aes(x = field, y = count)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "No. of Runners", y = "Count", title = "Field Size (9f dirt)")
...
ggplot(data = tmp, aes(x = gate, y = pct_btn)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "Gate", y = "Pct Beaten", title = "Percentage of Rivals Beaten (9f dirt)")
...
ggplot(tmp, aes(x = factor(gate), y = d9f)) +
    geom_boxplot(fill = fill) +
    RcappeR::theme_rcapper() +
    labs(x = "Gate", y = "Distance (ft)", title = "Distance Raced from each Starting Gate (9f dirt)")
...
y$variable <- factor(x = y$variable, levels = c("2f", "4f", "6f", "7f", "8f", "8.5f", "9f"))

ggplot(subset(y, pos ==1), aes(x = variable, y = value)) + 
    geom_boxplot() + 
    geom_jitter(position = position_jitter(width = .4), color=fill, alpha=.1) + 
    RcappeR::theme_rcapper() +
    labs(x = "Sectional", y = "Speed (ft per sec)", title="Sectional Speeds of Winners (9f dirt)")
...
names(tmp) = c("7f FinSpd", "5f FinSpd", "3f FinSpd", "2f FinSpd", "1f FinSpd", "0.5f FinSpd")
tmp
```
...
tmp2 <- tmp %>%
    ...
    summarise(c4f = round(mean(c4f, na.rm = TRUE),2),
              fs5f = round(mean(fs5f, na.rm = TRUE), 4))
...
ggplot(tmp, aes(x = c4f, y = fs5f, col = Winner)) + 
    geom_point(alpha = .5) + 
    scale_color_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) + 
    RcappeR::theme_rcapper() +
    labs(x = "Sectional (sec)", y = "Finishing Speed", title = "Opening 4f vs 4f Finishing Speed (9f dirt)")

fastestfs <- which.max(wnrs$fs5f)
fastestop <- which.min(wnrs$c4f)

```

The winner with the highest 5f Finishing Speed was **`r toupper(wnrs$horse[fastestfs])`** when winning a **`r wnrs$race_type[fastestfs]`** (race `r wnrs$race[fastestfs]` on `r wnrs$date[fastestfs]`), after an opening 4f of **`r wnrs$c4f[fastestfs]`** `r toupper(wnrs$horse[fastestfs])` recorded a Finishing Speed of **`r wnrs$fs5f[fastestfs]`**.  The winner with the fastest opening half mile was **`r toupper(wnrs$horse[fastestop])`** when winning a **`r wnrs$race_type[fastestop]`** (race `r wnrs$race[fastestop]` on `r wnrs$date[fastestop]`), recording a time of **`r wnrs$c4f[fastestop]`** over the opening half mile and a 5f Finishing Speed of **`r wnrs$fs5f[fastestop]`**.
...
ggplot(racetypes, aes(x = race_type, y = n)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "Race Type", y = "Count", title = "Counts per Race Type (9f turf)") +
    theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1, size = rel(1)))

ggplot(fieldsize, aes(x = field, y = count)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "No. of Runners", y = "Count", title = "Field Size (9f turf)")
...
ggplot(data = tmp, aes(x = gate, y = pct_btn)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "Gate", y = "Pct Beaten", title = "Percentage of Rivals Beaten (9f turf)")
...
ggplot(tmp, aes(x = factor(gate), y = d9f)) +
    geom_boxplot(fill = fill) +
    RcappeR::theme_rcapper() +
    labs(x = "Gate", y = "Distance (ft)", title = "Distance Raced from each Starting Gate (9f turf)")
...
Race shape on turf courses isn't quite as predictable as on dirt courses, as the surface allows horses who started slowly to finish faster (you will see the difference in profiles for 9f races on dirt and turf).  The plot below shows the sectional speeds of winners, to be clear these aren't cumulative speeds, but the speed from one sectional to the next (taking into account the distance travelled - Trakus data - for each horse), so from the start to 2f, from 2f to 3f, etc.  Winners are still typically running slower over the final 0.5f but the fall off isn't as sharp as seen on dirt, and the middle part of the race from 2f to ~8f sees runners accelerating before slowing in the final 0.5f.
...
ggplot(subset(y, pos ==1), aes(x = variable, y = value)) + 
    ...
    geom_jitter(position = position_jitter(width = .4), color=fill, alpha=.1) + 
    RcappeR::theme_rcapper() +
    labs(x = "Sectional", y = "Speed (ft per sec)", title="Sectional Speeds of Winners (9f turf)")
...
Finishing Speeds also reflect the consistent speed throughout the race, par Finishing Speeds for winners over the 9f Turf course are:
...
The plots below show the distribution of 5f Finishing Speeds for winners compared to "The Rest", and the opening 4f sectionals for winners compared to "The Rest".  The distributions for the opening 4f are very similar, Winners, on average, run the opening 4f in **`r win2f`**, compared to "The Rest", who run the opening 4f in **`r rest2f`**, a difference of **`r win_rest_diff`** seconds.  For reference, the horses leading their race after 4f run the opening 4f in an average time of **`r ldr2f`**.  The 5f Finishing Speeds are quite different, with "The Rest" having a much fatter distribution.  `r if(tmp2$fs5f[tmp2$Winner == "Winners"] >= 1) { paste("Winners average 5f Finishing Speed is above 1.000, meaning their final 5f is faster than their opening 4f, ie. they can just as easily win their races late.")}`
...
ggplot(tmp, aes(x = fs5f, y = ..density.. / 100, fill = Winner)) +
    ...
    scale_fill_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) +
    RcappeR::theme_rcapper() +
    labs(x = "Finishing Speed", y = "", title = "5f Finishing Speed density plot (9f turf)")

ggplot(tmp, aes(x = c4f, y = ..density.. / 100, fill = Winner)) +
    geom_density(alpha = .5) +
    scale_fill_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) +
    RcappeR::theme_rcapper() +
    labs(x = "Sectional (sec)", y = "", title = "Opening 4f Sectional density plot (9f turf)")
```

Plotting the opening 4f sectional against the closing 5f Finishing Speeds the difference becomes a little more clear.  Runners aren't necessarily punished if they start slowly as in sprint, or dirt, races, the kinder surface means horses are capable of finishing much faster and making up ground on horses who started faster.
...
ggplot(tmp, aes(x = c4f, y = fs5f, col = Winner)) + 
    ...
    scale_color_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) + 
    RcappeR::theme_rcapper() +
    labs(x = "Sectional (sec)", y = "Finishing Speed", title = "Opening 4f vs 5f Finishing Speed (9f turf)")
