...
ggplot(racetypes, aes(x = race_type, y = n)) +
    ...
    labs(x = "Race Type", y = "Count", title = "Counts per Race Type (6.5f dirt)") +
    theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1, size = rel(1)))

ggplot(fieldsize, aes(x = field, y = count)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "No. of Runners", y = "Count", title = "Field Size (6.5f dirt)")
...
ggplot(data = tmp, aes(x = gate, y = pct_btn)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(title = "Percentage of Rivals Beaten (6.5f dirt)")
...
tmp <- df %>%
    group_by(gate) %>%
    summarise(n = n(),
              mean_dist = mean(d6_5f, na.rm = TRUE))
...
ggplot(tmp, aes(x = factor(gate), y = d6_5f)) +
    geom_boxplot(fill = fill) +
    RcappeR::theme_rcapper() +
    labs(x = "Gate", y = "Distance (ft)", title = "Distance Raced from each Starting Gate (6.5f dirt)")
...
sspds <- select(df, pos, ss2f:ss6_5f)
...
y$variable <- factor(x = y$variable, levels = c("2f", "3f", "4f", "5f", "5.5f", "6f", "6.5f"))

ggplot(subset(y, pos ==1), aes(x = variable, y = value)) + 
    geom_boxplot() + 
    geom_jitter(position = position_jitter(width = .4), color=fill, alpha=.1) + 
    RcappeR::theme_rcapper() +
    labs(x = "Sectional", y = "Speed (ft per sec)", title="Sectional Speeds of Winners (6.5f dirt)")
...
names(tmp) = c("4.5f FinSpd", "3.5f FinSpd", "2.5f FinSpd", "1.5f FinSpd", "1f FinSpd", "0.5f FinSpd")
tmp
```
...
ggplot(tmp, aes(x = fs4_5f, y = ..density.. / 100, fill = Winner)) +
    ...
    RcappeR::theme_rcapper() +
    labs(x = "Finishing Speed", y = "", title = "4.5f Finishing Speed density plot (6.5f dirt)")

ggplot(tmp, aes(x = c2f, y = ..density.. / 100, fill = Winner)) +
    geom_density(alpha = .5) +
    scale_fill_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) +
    RcappeR::theme_rcapper() +
    labs(x = "Sectional (sec)", y = "", title = "Opening 2f Sectional density plot (6.5f dirt)")
```

Plotting the opening 2f sectional against the closing 4.5f Finishing Speeds the difference becomes a little more clear.  The cloud of points is crowded but winners are, for the majority, in the upper left part of the cloud.  

```{r echo=FALSE, warning=FALSE, message=FALSE, out.height="400px", out.width="550px", fig.align='center'}
ggplot(tmp, aes(x = c2f, y = fs4_5f, col = Winner)) + 
    geom_point(alpha = .5) + 
    scale_color_manual(values = c("Winners" = fill, "The Rest" = "#d8d8d8")) + 
    RcappeR::theme_rcapper() +
    labs(x = "Sectional (sec)", y = "Finishing Speed", title = "Opening 2f vs 4f Finishing Speed (6.5f dirt)")
