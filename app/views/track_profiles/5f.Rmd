...
ggplot(racetypes, aes(x = race_type, y = n)) +
    ...
    labs(x = "Race Type", y = "Count", title = "Counts per Race Type (5f turf)") +
    theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1, size = rel(1)))

ggplot(fieldsize, aes(x = field, y = count)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(x = "No. of Runners", y = "Count", title = "Field Size (5f turf)")
...
ggplot(data = tmp, aes(x = gate, y = pct_btn)) +
    geom_bar(fill = fill, stat = "identity") +
    RcappeR::theme_rcapper() +
    labs(title = "Percentage of Rivals Beaten (5f turf)")
...
tmp <- df %>%
    ...
    ungroup() %>%
    filter(runs >= 5)
ggplot(tmp, aes(x = factor(gate), y = d5f)) +
    geom_boxplot(fill = fill, aes = .75) +
    RcappeR::theme_rcapper() +
    labs(x = "Gate", y = "Distance (ft)", title = "Distance Raced from each Starting Gate (5f turf)")
...
ggplot(subset(y, pos ==1), aes(x = variable, y = value)) + 
    ...
    RcappeR::theme_rcapper() +
    labs(x = "Sectional", y = "Speed (ft per sec)", title="Sectional Speeds of Winners (5f turf)")
...
Finishing Speeds also reflect the slowing to the finish shown in the Race Shape plot above.  The par Finishing Speeds for `r dist`f winners are:
...
names(tmp) = c("3f FinSpd", "2f FinSpd", "1.5f FinSpd", "1f FinSpd", "0.5f FinSpd")
tmp
```
...
tmp2 <- tmp %>%
    ...
    summarise(c2f = round(mean(c2f, na.rm = TRUE),2),
              fs3f = round(mean(fs3f, na.rm = TRUE), 4))
