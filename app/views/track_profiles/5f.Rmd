...
ggplot(racetypes, aes(x = race_type, y = n)) +
  ...
  labs(x = "Race Type", y = "Count", title = "Race Types (5f Turf)") +
...
ggplot(data = pctbtn, aes(x = gate, y = pct_btn)) +
    ...
    RcappeR::theme_rcapper() +
    labs(title = "Percentage of Rivals Beaten (5f Turf)", y = "")
```
</div>
<div class="col-sm-6">
```{r echo=FALSE, warning=FALSE, message=FALSE, out.height="300px", out.width="500px", fig.align='center'}
...
ggplot(tmp, aes(x = factor(gate), y = d5f)) +
    geom_boxplot(fill = "#0BA42D", aes = .75) +
    RcappeR::theme_rcapper() +
    labs(x = "Gate", y = "Distance (ft)", title = "Distance Raced from each Starting Gate (5f Turf)")

rm(tmp, lowdist, lowgate, highdist, highgate, drawdist)
```
</div>
</div>
...
Finishing Speeds also reflect the slowing to the finish shown in the Race Shape plot above, ie. Finishing Speeds for both winners and losers is typically below 1.00.  The par Finishing Speeds for `r dist`f winners are:
...
ggplot(tmp, aes(x = fs3f, y = ..density.., fill = Winner)) +
