...
There are `r length(unique(mdnclm$date_race))` unique races, that require further grouping, as mentioned earlier the Claiming Price set for runners is used.  The ungrouped claiming prices for Maiden Claiming races is presented first, followed by the 5 groups that will be used:
...
The mean of each distribution per group will be 0, but typically the distribution will be right-skewed, this is due to ratings being a function of margins beaten, in turn a function of race times, which are similarly right skewed.

The ratings in the right-most tail are worthy of scrutiny and could be due to over-exaggerated margins back to well beaten horses in dirt races.  This could be mitigated (and should be in time) by calculating a lbs per second scale for each horse, taking into account the speed it ran its race (see **1.1**).  Alternatively, any horse whose beaten by some threshold could be ignored, but for the time being no horses are discarded like this.  With these races being Maiden Claiming races I am not too surprised by the longer right tails, as runners in these races will exhibit more variability in readyness, fitness, maturity, and ability.

A secondary issue that is likely affecting the ratings in the right most tail, is that the initialisation process treats each race in a group as equal, ie. the average ability of runners in each race is the same and they run under the same conditions.  This is clearly false, but the initialisation of handicap can't account for this, which is why this is only the first, initial, step.

### Ratings

As seen in the plot above, despite there being 5 different groups the initial ratings do not distinguish the groups, ie. the ratings are all around 0 and the difference in ability of horses competing within each group is not apparent.  This is possibly the biggest and most challenging of problems:

* What is the difference, in lbs, between an average winner of a Grade 1 Stakes race and an average winner of a Grade 2 Stakes race?



```{r echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE}
mdnclm %>% 
    filter(pos == 1, age == 3, dist <= 9) %>%
    group_by(surf, dist, cat) %>%
    mutate(ranktime = rank(fintime)) %>%
    filter(ranktime <= 20) %>%
    summarise(mean_time = mean(fintime), mean_wgt = mean(wgt)) %>%
    ungroup() %>%
    group_by(surf, dist) %>%
    mutate(btn_sec = btn_sec(mean_time),
           scale = lbs_per_sec(dist = dist, surf = surf[1]),
           btn_lbs = scale * btn_sec,
           diff_wgts = diff_at_wgts(btn_lbs = btn_lbs, wgt_carried = mean_wgt)) %>%
    ungroup() %>%
    group_by(cat) %>%
    summarise(diff = round(mean(diff_wgts))) %>%
    arrange(desc(diff))
```
...
**1.1**  -  The lbs per second scale I use is a version of one outlined by Bob Wilkins in his excellent book [Bioenergetics and Racehorse Ratings](http://www.amazon.co.uk/Bioenergetics-Racehorse-Ratings-Bob-Wilkins/dp/0956424309).  However ideally I believe each runner in a race should get its own lbs per second scale based on the speed at which it ran the race.  I've created a few lbs per second models but haven't been entirely happy, so for the time being I intend to adhere to the KISS principle (Keep It Simple Stupid).
...
* Which races do winners of a Maiden Claiming with a claiming price of $19k appear in compared to the winners of Maiden Special Weight with a purse of $43k?
...
**1.6**  -  The difference in performance between winners of one type of race and another is hugely important, even to the newest horse racing fan it should be apparent (just by prize money and prestige) that not all races are created equal.  The question is how **unequal** is one race compared to another.  

I think the solution of **1.4** could help solve this problem, instead of simple counts, the difference at the weights would be used.  So a horse who finished 2nd in a $43k Maiden Special Weight, beaten by 5lbs, then raced in a Maiden Claiming race, and won.  The difference in performance was 5lbs, averaging these differences over many races and runners could lead to the difference in ability of horses running in each races.

There is a section in Bob Wilkins' book ([Bioenergetics and Racehorse Ratings](http://www.amazon.co.uk/Bioenergetics-Racehorse-Ratings-Bob-Wilkins/dp/0956424309), chapter 4.3 to be precise) that spells out a method of using all races and performances to build collateral form lines, using all runners and all races.  Another application I recently read about was written by Martin Eastwood, [Massey Ratings for Football](http://pena.lt/y/2014/11/27/english-premier-league), which is a smaller but similar problem.  Any input would be welcomed.